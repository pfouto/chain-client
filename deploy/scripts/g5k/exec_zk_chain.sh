...
for run in $(# ------------------------------------------- RUN
  ...
  for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
    ...
    for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
      ...
      echo "strongRead=false" >>../apache-zookeeper-3.7.0-bin/conf/babel.conf
      ...
      for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
        ...
        exp_path_client="../logs/zk/${exp_name}/client/${n_servers}/${reads_per}/${payload}/chain/${run}"
        exp_path_server="../logs/zk/${exp_name}/server/${n_servers}/${reads_per}/${payload}/chain/${run}"
