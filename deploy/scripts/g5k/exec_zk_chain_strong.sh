...
for run in $(# ------------------------------------------- RUN
  ...
  for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
    ...
    for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
      ...
      echo "initial_membership=${servers_without_port}" >>../apache-zookeeper-3.7.0-bin/conf/babel.conf
      echo "quorum_size=${quorum_size}" >>../apache-zookeeper-3.7.0-bin/conf/babel.conf
      echo "strongRead=true" >>../apache-zookeeper-3.7.0-bin/conf/babel.conf

      for snode in "${server_nodes[@]}"; do
        oarsh "$snode" "rm -rf /tmp/zookeeper; mkdir /tmp/zookeeper"
      done
      ...
      for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
        ...
        exp_path_client="../logs/zk_strong/${exp_name}/client/${n_servers}/${reads_per}/${payload}/chain/${run}"
        exp_path_server="../logs/zk_strong/${exp_name}/server/${n_servers}/${reads_per}/${payload}/chain/${run}"
