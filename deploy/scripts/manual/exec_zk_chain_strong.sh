  ...
  for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
    ...
    for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
      ...
      for server_node in "${server_nodes[@]}"; do
        ...
        ssh "$server_node" "echo strongRead=true >> $HOME/chainpaxos/apache-zookeeper-3.7.0-bin/conf/babel.conf"
      done

      for snode in "${server_nodes[@]}"; do
        ssh "$snode" "rm -rf /tmp/zookeeper; mkdir /tmp/zookeeper"
      done
      ...
      for server_node in "${server_nodes[@]}"; do
        ssh "$server_node" "cd chainpaxos/apache-zookeeper-3.7.0-bin && \
            bin/zkServer.sh start-foreground zoo_sample.cfg" 2>&1 | sed "s/^/[s-$server_node] /" &
        server_p_ids+=($!)
        sleep 0.2
      done
      sleep 10
      ...
      for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
        ...
        exp_path_client="$HOME/chainpaxos/logs/zk_strong/${exp_name}/client/${n_servers}/${reads_per}/${payload}/chain/${run}"
        exp_path_server="$HOME/chainpaxos/logs/zk_strong/${exp_name}/server/${n_servers}/${reads_per}/${payload}/chain/${run}"
