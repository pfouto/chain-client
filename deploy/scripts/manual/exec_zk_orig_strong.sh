  ...
  for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
    ...
    for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
      ...
      for server_node in "${server_nodes[@]}"; do
        ssh "$server_node" "cp $HOME/chainpaxos/apache-zookeeper-3.7.0-bin/conf/zoo_sample.cfg $HOME/chainpaxos/apache-zookeeper-3.7.0-bin/conf/orig.cfg"
        i=1
        for snode in "${server_nodes[@]}"; do
          ssh "$server_node" "echo server.${i}=${snode}:2888:3888 >> $HOME/chainpaxos/apache-zookeeper-3.7.0-bin/conf/orig.cfg"
          i=$((i + 1))
        done
      done
      ...
      i=1
      for snode in "${server_nodes[@]}"; do
        ssh "$snode" "rm -rf /tmp/zookeeper; mkdir /tmp/zookeeper && echo ${i} > /tmp/zookeeper/myid"
        i=$((i + 1))
      done
      ...
      server_p_ids=()
      for server_node in "${server_nodes[@]}"; do
        ssh "$server_node" "cd chainpaxos/apache-zookeeper-3.7.0-bin && \
            bin/zkServer.sh start-foreground ${OAR_JOB_ID}.cfg" 2>&1 | sed "s/^/[s-$server_node] /" &
        server_p_ids+=($!)
      done
      sleep 8
      echo -e "$BLUE Loading data $NC"
      ssh "${client_nodes[0]}" "cd chainpaxos/client && java -cp chain-client.jar:. \
      ...
      backup_pids=()
      for server_node in "${server_nodes[@]}"; do
        ssh "$server_node" "mkdir /tmp/zookeeper_bak; rm -r /tmp/zookeeper_bak/*; cp -r /tmp/zookeeper/* /tmp/zookeeper_bak/" &
        backup_pids+=($!)
      done
      ...
      for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
        ...
        exp_path_client="$HOME/chainpaxos/logs/zk_strong/${exp_name}/client/${n_servers}/${reads_per}/${payload}/original/${run}"
        exp_path_server="$HOME/chainpaxos/logs/zk_strong/${exp_name}/server/${n_servers}/${reads_per}/${payload}/original/${run}"
        ...
        for n_threads in "${threads_list[@]}"; do # -------------------- N_THREADS
          ...
          for server_node in "${server_nodes[@]}"; do
            ssh "$server_node" "rm -r /tmp/zookeeper/* ; cp -r /tmp/zookeeper_bak/* /tmp/zookeeper/" &
            backup_pids+=($!)
          done
          for pid in "${backup_pids[@]}"; do
            wait "$pid"
          ...
          for server_node in "${server_nodes[@]}"; do
            ssh "$server_node" "cd chainpaxos/apache-zookeeper-3.7.0-bin && \
            bin/zkServer.sh start-foreground orig.cfg" 2>&1 | sed "s/^/[s-$server_node] /" &
            server_p_ids+=($!)
          done
