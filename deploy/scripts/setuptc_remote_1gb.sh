#!/bin/bash
if [[ -z "$2" ]]; then
  echo "Usage: setup_remove_1gb.sh [nServers] [latency_file]"
  exit
fi
...
while read -r raw_line; do
  ...
  print_and_exec "${nodes[$i]}" "sudo-g5k tc qdisc del dev br0 root; sudo-g5k tc qdisc add dev br0 root handle 1: htb default 1"
  ...
  for n in "${line[@]:2}"; do
    if [ $i -ne $j ]; then
      ...
      cmd1="sudo-g5k tc class add dev br0 parent 1:1 classid 1:1$j htb rate 200mbit ceil 20000mbit && "
