      ...
      for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
        ...
        for alg in "${algs_list[@]}"; do # ----------------------------------- ALG
          ...
          for n_threads in "${threads_list[@]}"; do # -------------------- N_THREADS
            ...
            oarsh "$node" "cd chainpaxos/client && java -cp chain-client.jar \
                      site.ycsb.Client -t -s -P config.properties \
											-threads 30 -p fieldlength=$payload \
											-p hosts=$servers_without_port -p n_frontends=1 -p readproportion=${reads_per} \
											-p insertproportion=${writes_per}" 2>&1 | sed "s/^/[c-$node [extra]] /" &
              client_p_ids+=($!)
