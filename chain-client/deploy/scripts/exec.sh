...
if [[ -z "${n_frontends_arg}" ]]; then
  echo "n_frontends not set"
  exit
fi
...
IFS=', ' read -r -a batchsize_list <<<"$batch_size_arg"
IFS=', ' read -r -a n_servers_list <<<"$n_servers_arg"
IFS=', ' read -r -a algs_list <<<"$algs_arg"
IFS=', ' read -r -a reads_list <<<"$reads_arg"
IFS=', ' read -r -a frontends_list <<<"$n_frontends_arg"
IFS=', ' read -r -a threads_list <<<"$n_threads_arg"

total_runs=$((n_runs * ${#payloads_list[@]} * ${#batchsize_list[@]} * ${#n_servers_list[@]} * ${#algs_list[@]} * ${#reads_list[@]} * ${#frontends_list[@]} * ${#threads_list[@]}))

# ----------------------------------- LOG PARAMS ------------------------------
echo -e "$BLUE\n ---- CONFIG ----  $NC"
echo -e "$GREEN exp_name: $NC \t\t\t${exp_name}"
echo -e "$GREEN jar_file: $NC \t\t\t${jar_file}"
echo -e "$GREEN main_class: $NC \t\t\t${main_class}"
...
echo -e "$GREEN batches: $NC \t\t\t${batchsize_list[*]}"
echo -e "$GREEN algorithms:  $NC \t\t\t${algs_list[*]}"
echo -e "$GREEN frontends: $NC \t\t\t${frontends_list[*]}"
...
for run in $(seq "$start_run" $((n_runs + start_run - 1))); do # ------------------------------------------- RUN
  ...
  for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
    ...
    for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
      ...
      for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
        ...
        for batchsize in "${batchsize_list[@]}"; do # ------------------- BATCH_SIZE
          echo -e "$GREEN -- -- -- -- -- STARTING BATCHSIZE $NC$batchsize"

          for alg in "${algs_list[@]}"; do # ----------------------------------- ALG
            echo -e "$GREEN -- -- -- -- -- -- STARTING ALG $NC$alg"

            for frontends in "${frontends_list[@]}"; do # ---------------- FRONTENDS
              echo -e "$GREEN -- -- -- -- -- -- -- STARTING FRONTENDS $NC$frontends"

              exp_path_client="../logs/${exp_name}/client/${n_servers}/${reads_per}/${payload}/${batchsize}/${alg}/${frontends}/${run}"
              exp_path_server="../logs/${exp_name}/server/${n_servers}/${reads_per}/${payload}/${batchsize}/${alg}/${frontends}/${run}"
              ...
              for n_threads in "${threads_list[@]}"; do # -------------------- N_THREADS
                ...
                for node in "${client_nodes[@]}"; do
                  oarsh "$node" "cd chainpaxos/client && java -Dlog4j.configurationFile=log4j2.xml \
											...
											-p hosts=$servers_without_port -p n_frontends=$frontends \
